...
def run_one(spec):
    family, symbol, mode, output, checkpoint = spec
...
def main():
    ap = argparse.ArgumentParser(); ap.add_argument("--output", default="docs/evidence/phase3/search"); args = ap.parse_args()
    out = ROOT / args.output; out.mkdir(parents=True, exist_ok=True)
    specs = []
    for mode in ("stratified", "random"):
        for family, symbol in CASES:
            stem = f"{mode}_{family.replace('-', '_')}_{symbol.lower()}"
            specs.append((family, symbol, mode, out / f"{stem}.json", out / f"{stem}.checkpoint.json"))
    with ThreadPoolExecutor(max_workers=4) as pool:
        results = list(pool.map(run_one, specs))
    grouped = {}
    for result in results: grouped.setdefault(result["mode"], []).append(result)
    merged = {"schema_version": "nora.phase3.execution_result_v1", "protocol": "docs/evidence/phase3/phase3_protocol_v1.json", "workers": 4, "batch_size": 250, "guided": {}, "matched_random_baseline": {}, "lockbox_touched": False}
    ...
    merged["coverage_assessment"] = {"declared_minimum_cells": 20, "observed_cells": merged["guided"]["archive_cells"], "passes": merged["guided"]["archive_cells"] >= 20}
    (out / "phase3_execution_result_v1.json").write_text(json.dumps(merged, sort_keys=True, indent=2) + "\n")
