"""Phase-2R durable native evidence upgrade for the condition canary.

This test defines the acceptance boundary for bringing
``canary.condition_native`` from ``legacy_committed_summary`` up to
``self_contained_raw_native`` evidence-package completeness — the same
standard Phase-2N slope and Phase-2Q ATR/Distance-ATR already meet.
...
NATIVE_ROOT = ROOT / "tests/fixtures/phase2r_mql5_condition_native"
NATIVE_INDEX = NATIVE_ROOT / "native_evidence_manifest.json"

# Frozen identities inherited from the accepted legacy condition canary.
# These pin the contract: the durable package must reconcile against the same
# frozen source and semantic result, not a new one.
...
LEGACY_SEMANTIC_RESULT_IDENTITY = "b66f60ad5ae4cc036d29197063e2dbe355cafac96085c359e92783ac74da74e4"
...
class Phase2RConditionNativeEvidenceStructuralTests(unittest.TestCase):
    ...
    def test_index_schema_is_self_contained_raw_native(self):
        ...
        # Evidence version must declare the durable v2 standard, not the legacy summary.
        self.assertEqual(index["evidence_version"], "nora.phase2r.native_evidence_v2")
        self.assertEqual(index["scope"], "frozen native condition canary only")
        # Compile artifacts
        for key in ("compile", "compile_log", "ex5"):
            self.assertIn(key, index, f"index missing compile artifact key {key!r}")
        # Two runs required for determinism proof
        ...
        # Source hashes pin the frozen tester source
        self.assertIn("source_hashes", index)
        self.assertIn("tester_sha256", index["source_hashes"])
        # Determinism flag must be asserted
    ...
    def test_frozen_tester_source_hash_is_bound_and_matches(self):
        index = json.loads(NATIVE_INDEX.read_text())
        self.assertEqual(
            index["source_hashes"]["tester_sha256"],
            TESTER_SOURCE_SHA256,
            "durable package must bind the frozen tester source hash, not a new one",
        )


class Phase2RConditionNativeCompileTests(unittest.TestCase):
    """Compile evidence must be self-contained and freshness-bound."""

    def setUp(self):
        if not NATIVE_INDEX.is_file():
            self.skipTest("Phase 2R condition native evidence not yet produced")
    ...
    def test_tester_source_hash_bound_in_compile(self):
        self.assertEqual(self.compile["tester_source_sha256"], TESTER_SOURCE_SHA256)


class Phase2RConditionNativeRunTests(unittest.TestCase):
    ...
    def test_each_run_tester_ini_is_redacted(self):
        for number in (1, 2):
            ...
            self.assertNotIn("4000094575", config, f"run{number} tester.ini leaked account number")

    def test_each_run_tester_log_is_committed_and_nonempty(self):
        # The execution_manifest references tester.log under the run root.
        for number in (1, 2):
            run_root = (NATIVE_ROOT / self.index[f"run{number}"]).parent
    ...
    def test_each_run_reconciles_to_frozen_vectors(self):
        for number, run in enumerate(self.runs, 1):
...
class Phase2RConditionNativeDeterminismTests(unittest.TestCase):
