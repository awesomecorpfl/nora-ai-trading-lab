...
def test_runner_uses_the_persistent_evidence_root_and_sid_acl_contract():
    ...
    assert "pre-existing evidence changed during ACL hardening" in RUNNER
...
def test_persistent_orchestrator_uses_only_the_repository_runner_commands():
    for mode in ("harden-acl", "prepare", "launch", "status", "retrieve", "package-persistent", "record-import"):
