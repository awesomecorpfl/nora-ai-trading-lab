...
fn simulate(strategy:&Map<String,Value>,segment:&Map<String,Value>)->Result<Value>{
 ...
 let bracket=req(strategy,"brackets")?.as_object().ok_or("brackets")?;if text(bracket,"construction")?!="fixed_embedded_price_offset"{return Err("bracket construction".into())}let stop_offset=number(bracket,"stop_price_offset")?;let target_offset=number(bracket,"target_price_offset")?;
 ...
 for i in 0..bs.len(){
  if let Some(source)=pending.take(){if position.is_none(){let entry=bs[i].open;position=Some((source,i,entry,if long{entry-stop_offset}else{entry+stop_offset},if long{entry+target_offset}else{entry-target_offset}));}}
