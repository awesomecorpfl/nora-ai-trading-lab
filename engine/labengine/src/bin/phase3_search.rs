...
use std::{collections::{BTreeSet, HashMap}, env, fs::File, path::Path};
...
fn evaluate(m:&Market,c:&Candidate,emas:&mut HashMap<usize,Vec<f64>>,atrs:&mut HashMap<usize,Vec<f64>>,roll:&mut HashMap<(usize,bool),Vec<f64>>)->(usize,Option<f64>,Option<f64>,HashMap<String,String>,String){
    ...
    let mut open:Option<(usize,f64,f64)>=None; let mut pnls=Vec::new(); let mut holds=Vec::new(); let mut longs=0usize; let mut hours=BTreeSet::new();
