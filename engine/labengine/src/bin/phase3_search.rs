...
#[derive(Clone, Serialize, Deserialize)] struct Candidate { family: String, symbol: String, side: String, fast: usize, slow: usize, atr_period: usize, distance_limit: f64, slope_lookback: usize, lookback: usize, max_bars: usize, stop_atr: f64, target_atr: f64, breakout_buffer: f64 }
...
fn aggregate(m:&Market,tf:&str)->Market{let n=if tf=="M5"{5}else if tf=="H1"{60}else{1};if n==1{return m.clone()}let mut ts=Vec::new();let(mut o,mut h,mut l,mut c)=(Vec::new(),Vec::new(),Vec::new(),Vec::new());let mut i=0;while i+n<=m.c.len(){ts.push(m.ts[i].clone());o.push(m.o[i]);h.push(m.h[i..i+n].iter().copied().fold(f64::NEG_INFINITY,f64::max));l.push(m.l[i..i+n].iter().copied().fold(f64::INFINITY,f64::min));c.push(m.c[i+n-1]);i+=n;}Market{ts,o,h,l,c}}
...
fn candidate_v2(family:&str,symbol:&str,n:usize)->Candidate{
    let mut c=candidate(family,symbol,n);
    c.side=if n%2==0{"long".into()}else{"short".into()};
    c.max_bars=[8,16,32][(n/2)%3];
    c.stop_atr=[0.5,1.0,1.5][(n/6)%3];
    c.target_atr=[1.0,2.0,3.0][(n/18)%3];
    c
}
...
fn stable_metric(v:f64)->f64{(v*1_000_000_000_000_000.).round()/1_000_000_000_000_000.}
fn structural_descriptor(c:&Candidate)->(BTreeMap<String,String>,String){
    let mut d=BTreeMap::new();
    d.insert("trigger_shape".into(),c.family.clone());
    d.insert("side".into(),c.side.clone());
    d.insert("holding_policy".into(),match c.max_bars {8=>"short",16=>"medium",32=>"long",_=>"other"}.into());
    let ratio=c.target_atr/c.stop_atr;
    d.insert("risk_shape".into(),if ratio<1.5{"sub_one_to_one"}else if ratio<2.5{"balanced"}else{"target_dominant"}.into());
    let cell=canon(&d);(d,cell)
}
fn evaluate(m:&Market,c:&Candidate,emas:&mut HashMap<usize,Vec<f64>>,atrs:&mut HashMap<usize,Vec<f64>>,roll:&mut HashMap<(usize,bool),Vec<f64>>)->(usize,Option<f64>,Option<f64>,BTreeMap<String,String>,String,Vec<f64>,Vec<String>){
    ...
    let mut open:Option<(usize,f64,f64)>=None; let mut pnls=Vec::new(); let mut trade_ts=Vec::new(); let mut holds=Vec::new(); let mut longs=0usize; let mut hours:HashMap<String,usize>=HashMap::new();
    for i in 1..m.c.len(){let valid=atrs_v[i].is_finite();if !valid{continue}let entry=if c.family=="trend-pullback"{let slope=if i>=c.slope_lookback{(slow[i]-slow[i-c.slope_lookback])/c.slope_lookback as f64}else{f64::NAN};let d=(m.c[i]-fast[i]).abs()/atrs_v[i];if !fast[i].is_finite()||!slow[i].is_finite()||!slope.is_finite(){false}else if c.side=="long"{m.c[i-1]<=fast[i-1]&&m.c[i]>fast[i]&&fast[i]>slow[i]&&slope>0.&&d<=c.distance_limit}else{m.c[i-1]>=fast[i-1]&&m.c[i]<fast[i]&&fast[i]<slow[i]&&slope<0.&&d<=c.distance_limit}}else{let (hi,lo)=levels.as_ref().unwrap();if !hi[i].is_finite(){false}else if c.side=="long"{m.c[i-1]<=hi[i-1]&&m.c[i]>hi[i]*(1.+c.breakout_buffer/10000.)}else{m.c[i-1]>=lo[i-1]&&m.c[i]<lo[i]*(1.-c.breakout_buffer/10000.)}};
        if let Some((ei,price,entry_atr))=open {let mut close=None;if i-ei>=c.max_bars{close=Some(price*0.+m.o[i])}else{let(stop,target)=if c.side=="long"{(price-entry_atr*c.stop_atr,price+entry_atr*c.target_atr)}else{(price+entry_atr*c.stop_atr,price-entry_atr*c.target_atr)};if c.side=="long"{if m.o[i]<=stop{close=Some(m.o[i])}else if m.o[i]>=target{close=Some(m.o[i])}else if m.l[i]<=stop{close=Some(stop)}else if m.h[i]>=target{close=Some(target)}else if !entry{continue}}else if m.o[i]>=stop{close=Some(m.o[i])}else if m.o[i]<=target{close=Some(m.o[i])}else if m.h[i]>=stop{close=Some(stop)}else if m.l[i]<=target{close=Some(target)}else if !entry{continue}}if let Some(exit)=close{let pnl=if c.side=="long"{exit-price}else{price-exit};pnls.push(pnl);trade_ts.push(m.ts[ei].clone());holds.push(i-ei);*hours.entry(m.ts[ei].get(11..13).unwrap_or("00").to_string()).or_insert(0)+=1;if c.side=="long"{longs+=1}open=None;}}
        if open.is_none()&&entry{open=Some((i,m.o[i],atrs_v[i]));}
    }
    if pnls.is_empty(){return (0,None,None,BTreeMap::new(),"empty".into(),pnls,trade_ts)}let avg=stable_metric(pnls.iter().sum::<f64>()/pnls.len()as f64);let mut equity: f64=0.;let mut peak: f64=0.;let mut dd: f64=0.;for p in &pnls{equity+=p;peak=peak.max(equity);dd=dd.max(peak-equity)}let dd_frac=stable_metric(dd/m.o[0].abs());let n=pnls.len();let hold=holds.iter().sum::<usize>()as f64/n as f64;let long_frac=longs as f64/n as f64;let months=(m.ts.last().map(|x|x.get(0..7).unwrap_or("2024")).unwrap_or("2024"),m.ts.first().map(|x|x.get(0..7).unwrap_or("2024")).unwrap_or("2024"));let months_count=if months.0==months.1{1.}else{12.};let trades_month=n as f64/months_count;let top_session=hours.values().copied().max().unwrap_or(0)as f64/n as f64;let mut d=BTreeMap::new();d.insert("trades_per_month".into(),bucket(trades_month,2.,8.));d.insert("holding_period".into(),bucket(hold,5.,20.));d.insert("long_short_balance".into(),bucket(long_frac,0.33,0.67));d.insert("session_concentration".into(),bucket(top_session,0.10,0.25));let cell=canon(&d);(n,Some(avg),Some(dd_frac),d,cell,pnls,trade_ts)
}
fn candidate_v2_for_index(family:&str,symbol:&str,mode:&str,index:usize)->Candidate{
    if index<100000{return candidate_v2(family,symbol,index)}
    let step=index-100000;let parent_idx=if mode=="random"{(step.wrapping_mul(7919).wrapping_add(104729))%2500}else{step%2000};refine(candidate_v2(family,symbol,parent_idx),step)
}
fn main(){
    let a:Vec<String>=env::args().collect(); let get=|k:&str|a.iter().position(|x|x==k).and_then(|i|a.get(i+1)).cloned().unwrap_or_default();
    let input=get("--input"); let family=get("--family"); let symbol=get("--symbol"); let mode=get("--mode"); let output=get("--output"); let checkpoint=get("--checkpoint"); let phase4_input=get("--phase4-input"); let phase4_is=get("--is-input"); let phase4_oos=get("--oos-input");
    let archive_version=get("--archive-version");
    if !phase4_input.is_empty(){
        let lower=|p:&str|p.to_ascii_lowercase(); if lower(&phase4_is).contains("lockbox")||lower(&phase4_oos).contains("lockbox"){eprintln!("Phase-4 evaluator refuses lockbox input");std::process::exit(2)}
        let is_market=match read(Path::new(&phase4_is)){Ok(x)=>x,Err(e)=>{eprintln!("{e}");std::process::exit(2)}};let oos_market=match read(Path::new(&phase4_oos)){Ok(x)=>x,Err(e)=>{eprintln!("{e}");std::process::exit(2)}};
        let specs:Vec<serde_json::Value>=match serde_json::from_str(&std::fs::read_to_string(&phase4_input).unwrap()){Ok(x)=>x,Err(e)=>{eprintln!("{e}");std::process::exit(2)}};let mut out_rows=Vec::new();
        for s in specs{let fam=s["family"].as_str().unwrap();let sym=s["symbol"].as_str().unwrap();let md=s["mode"].as_str().unwrap_or("stratified");let idx=s["candidate_index"].as_u64().unwrap()as usize;let c=if s.get("candidate_override").is_some(){serde_json::from_value(s["candidate_override"].clone()).unwrap()}else{candidate_v2_for_index(fam,sym,md,idx)};let tf=s.get("timeframe").and_then(|x|x.as_str()).unwrap_or("M1");let compact=s.get("compact").and_then(|x|x.as_bool()).unwrap_or(false);let is_eval=aggregate(&is_market,tf);let oos_eval=aggregate(&oos_market,tf);let mut ie=HashMap::new();let mut ia=HashMap::new();let mut ir=HashMap::new();let mut oe=HashMap::new();let mut oa=HashMap::new();let mut orr=HashMap::new();let(in_n,in_avg,in_dd,_,_,in_pnl,in_ts)=evaluate(&is_eval,&c,&mut ie,&mut ia,&mut ir);let(out_n,out_avg,out_dd,_,_,out_pnl,out_ts)=evaluate(&oos_eval,&c,&mut oe,&mut oa,&mut orr);out_rows.push(json!({"source":md,"family":fam,"symbol":sym,"timeframe":tf,"candidate_index":idx,"candidate_identity":s["candidate_identity"],"state":s["state"],"descriptor":s["descriptor"],"candidate":c,"is_initial_price":is_eval.o.first().copied(),"oos_initial_price":oos_eval.o.first().copied(),"is_trades":in_n,"is_average_trade":in_avg,"is_drawdown_fraction":in_dd,"is_pnls":if compact{Vec::new()}else{in_pnl},"is_trade_timestamps":if compact{Vec::new()}else{in_ts},"oos_trades":out_n,"oos_average_trade":out_avg,"oos_drawdown_fraction":out_dd,"oos_pnls":if compact{Vec::new()}else{out_pnl},"oos_trade_timestamps":if compact{Vec::new()}else{out_ts}}));}
        std::fs::write(&output,serde_json::to_string(&json!({"schema_version":"nora.phase4.evaluation_result_v1","rows":out_rows})).unwrap()).unwrap();return
    }
    ...
    let v2_mode=archive_version.starts_with("v2"); let strategy_domain=if v2_mode{"nora.phase3.strategy.v2"}else{"nora.phase3.strategy.v1"}; let template_domain=if v2_mode{"nora.phase3.template.v2"}else{"nora.phase3.template.v1"};
    let start=rows.len(); for trial in start..total {let (idx,c,parent)=if trial<sampled{let idx=if mode=="random"{(trial.wrapping_mul(7919).wrapping_add(104729))%100000}else{trial};let c=if v2_mode{candidate_v2(&family,&symbol,idx)}else{candidate(&family,&symbol,idx)};(idx,c,None)}else{let step=trial-sampled;let parent_idx=if mode=="random"{(step.wrapping_mul(7919).wrapping_add(104729))%sampled.max(1)}else{step%sampled.max(1)};let parent=if v2_mode{candidate_v2(&family,&symbol,parent_idx)}else{candidate(&family,&symbol,parent_idx)};let parent_id=id(strategy_domain,&canon(&parent));(100000+step,refine(parent,step),Some(parent_id))};let cid=id(strategy_domain,&canon(&c));if let Some(prior)=seen.get(&cid){rows.push(ResultRow{trial,candidate_index:idx,candidate_identity:cid,family:family.clone(),symbol:symbol.clone(),state:"duplicate_rejected".into(),duplicate_of:Some(prior.clone()),trades:0,average_trade:None,max_drawdown_fraction:None,descriptor:BTreeMap::new(),archive_cell:"".into(),lineage:BTreeMap::new()});}else{let(n,avg,dd,mut desc,mut cell,_,_)=evaluate(&market,&c,&mut emas,&mut atrs,&mut roll);if v2_mode{(desc,cell)=structural_descriptor(&c);}let pass=n>=30&&dd.map(|x|x<=0.10).unwrap_or(false);let mut lineage=BTreeMap::new();lineage.insert("template_identity".into(),id(template_domain,&family));lineage.insert("parent_identity".into(),parent.unwrap_or_else(||"root_stratified_template".into()));lineage.insert("sampler_mode".into(),mode.clone());lineage.insert("stage".into(),if trial<sampled{"stratified_sample_v2".into()}else{"local_refinement_v2".into()});rows.push(ResultRow{trial,candidate_index:idx,candidate_identity:cid.clone(),family:family.clone(),symbol:symbol.clone(),state:if pass{"accepted".into()}else{"rejected_threshold".into()},duplicate_of:None,trades:n,average_trade:avg,max_drawdown_fraction:dd,descriptor:desc,archive_cell:cell,lineage});seen.insert(cid.clone(),cid);}
