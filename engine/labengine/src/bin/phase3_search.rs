...
fn main(){
    ...
    if !phase4_input.is_empty(){
        ...
        for s in specs{let fam=s["family"].as_str().unwrap();let sym=s["symbol"].as_str().unwrap();let md=s["mode"].as_str().unwrap_or("stratified");let idx=s["candidate_index"].as_u64().unwrap()as usize;let c=if s.get("candidate_override").is_some(){serde_json::from_value(s["candidate_override"].clone()).unwrap()}else{candidate_v2_for_index(fam,sym,md,idx)};let tf=s.get("timeframe").and_then(|x|x.as_str()).unwrap_or("M1");let is_eval=aggregate(&is_market,tf);let oos_eval=aggregate(&oos_market,tf);let mut ie=HashMap::new();let mut ia=HashMap::new();let mut ir=HashMap::new();let mut oe=HashMap::new();let mut oa=HashMap::new();let mut orr=HashMap::new();let(in_n,in_avg,in_dd,_,_,in_pnl,in_ts)=evaluate(&is_eval,&c,&mut ie,&mut ia,&mut ir);let(out_n,out_avg,out_dd,_,_,out_pnl,out_ts)=evaluate(&oos_eval,&c,&mut oe,&mut oa,&mut orr);out_rows.push(json!({"source":md,"family":fam,"symbol":sym,"timeframe":tf,"candidate_index":idx,"candidate_identity":s["candidate_identity"],"state":s["state"],"descriptor":s["descriptor"],"candidate":c,"is_initial_price":is_eval.o.first().copied(),"oos_initial_price":oos_eval.o.first().copied(),"is_trades":in_n,"is_average_trade":in_avg,"is_drawdown_fraction":in_dd,"is_pnls":in_pnl,"is_trade_timestamps":in_ts,"oos_trades":out_n,"oos_average_trade":out_avg,"oos_drawdown_fraction":out_dd,"oos_pnls":out_pnl,"oos_trade_timestamps":out_ts}));}
