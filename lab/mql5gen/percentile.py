...
int OnInit(){double actual[];bool actual_null[];ArrayResize(actual,NORA_PERCENTILE_ROWS);ArrayResize(actual_null,NORA_PERCENTILE_ROWS);if(!NoraPhase2Percentile(source,source_null,NORA_PERCENTILE_ROWS,4,actual,actual_null)){Print("NORA_PHASE2W_PERCENTILE_FAIL");return INIT_FAILED;}int f=FileOpen(NORA_PERCENTILE_CSV,FILE_WRITE|FILE_CSV|FILE_ANSI);if(f==INVALID_HANDLE)return INIT_FAILED;FileWrite(f,"row","source","percentile","pass");for(int i=0;i<NORA_PERCENTILE_ROWS;i++){bool ok=actual_null[i]==expected_null[i]&&(actual_null[i]||MathAbs(actual[i]-expected[i])<=1e-12);FileWrite(f,i,NoraPercentileCsv(source[i],source_null[i]),NoraPercentileCsv(actual[i],actual_null[i]),ok?"true":"false");if(!ok){FileClose(f);Print("NORA_PHASE2W_PERCENTILE_FAIL");return INIT_FAILED;}}FileClose(f);Print("'''+COMPLETION+'''");return INIT_SUCCEEDED;}
