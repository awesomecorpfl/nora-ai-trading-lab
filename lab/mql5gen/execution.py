...
def _tester(records):
 ...
 return ('''#property strict\n#include "NoraPhase2ExecutionRuntimeV1.mqh"\nstruct NoraExpectedExecutionV1 { string id; int row; int entry_index; double entry_price; int exit_index; double exit_price; string side; double stop; double target; string reason; string state; };\nNoraExpectedExecutionV1 NoraExpected[%d]={%s};\nstring NoraExecutionCsv(const double v,const bool null_value){return null_value?"NULL":DoubleToString(v,16);}\nint OnInit(){int f=FileOpen("%s",FILE_WRITE|FILE_CSV|FILE_ANSI,'\\t');if(f==INVALID_HANDLE){Print("%s");return INIT_FAILED;}FileWrite(f,"scenario_id","ledger_row_index","entry_bar_index","entry_price","exit_bar_index","exit_price","direction","stop_price","target_price","exit_reason","expected_state","pass");for(int i=0;i<ArraySize(NoraExpected);i++){NoraExpectedExecutionV1 e=NoraExpected[i];NoraExecutionResultV1 actual=NoraExecutionResolveV1(e.entry_price,e.entry_index,e.exit_price,e.exit_price,e.exit_price,false,false,e.reason=="signal",e.reason=="max_bars_held",e.reason=="initial_stop"||e.reason=="initial_stop_pessimistic",e.reason=="initial_target");bool pass=(actual.reason==e.reason || e.reason=="no_trade");FileWrite(f,e.id,e.row,e.entry_index<0?"NULL":IntegerToString(e.entry_index),NoraExecutionCsv(e.entry_price,e.entry_index<0),e.exit_index<0?"NULL":IntegerToString(e.exit_index),NoraExecutionCsv(e.exit_price,e.exit_index<0),e.side,NoraExecutionCsv(e.stop,e.entry_index<0),NoraExecutionCsv(e.target,e.entry_index<0),e.reason,e.state,pass?"true":"false");if(!pass){FileClose(f);Print("%s");return INIT_FAILED;}}FileClose(f);Print("%s");return INIT_SUCCEEDED;}void OnDeinit(const int reason){}\n'''%(len(rows),','.join(rows),CSV,FAIL,FAIL,MARKER)).encode()
