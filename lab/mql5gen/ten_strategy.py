...
void WriteNoTrade(int f,string sid,string direction,string reason){FileWrite(f,sid,"NULL",direction,"NULL","NULL","NULL","NULL","NULL","NULL","NULL","NULL","NULL","NULL","NULL","NULL","NULL",reason,"none");}
void RunCase(int f,string sid,string family,string direction,int period,double limit,int maxhold,bool filterroll,bool filtermon,const NoraBar &b[]){
 ...
 bool open=false,pending=false,suppressed=false;int source=-1,entry=-1,ordinal=0;double ep=0,stop=0,target=0;for(int i=0;i<z;i++){if(pending){entry=i;ep=b[i].o;stop=direction=="long"?ep-4.0:ep+4.0;target=direction=="long"?ep+6.0:ep-6.0;open=true;pending=false;}if(open&&i>entry){bool gs=direction=="long"?b[i].o<=stop:b[i].o>=stop,gt=direction=="long"?b[i].o>=target:b[i].o<=target,sh=direction=="long"?b[i].l<=stop:b[i].h>=stop,th=direction=="long"?b[i].h>=target:b[i].l<=target;double xp=0;string reason="";if(gs){xp=b[i].o;reason="gap_stop";}else if(gt){xp=b[i].o;reason="gap_target";}else if(opposite[i]){xp=b[i].c;reason="signal_exit";}else if(b[i].friday||i-entry>=maxhold){xp=b[i].c;reason=b[i].friday?"friday_close":"time_exit";}else if(sh&&th){xp=stop;reason="pessimistic_dual_touch";}else if(sh){xp=stop;reason="stop";}else if(th){xp=target;reason="target";}if(reason!=""){ordinal++;FileWrite(f,sid,ordinal,direction,source,b[source].ts,entry,b[entry].ts,DoubleToString(ep,16),DoubleToString(stop,16),DoubleToString(target,16),i,b[i].ts,DoubleToString(xp,16),reason,i-entry,DoubleToString(direction=="long"?xp-ep:ep-xp,16),"NULL","not_executed");open=false;}}
 if(!open&&!pending&&signal[i]){if(!b[i].session|| (filterroll&&b[i].rollover)|| (filtermon&&b[i].monday)){suppressed=true;}else if(i+1<z){pending=true;source=i;}}}
 if(ordinal==0)WriteNoTrade(f,sid,direction,suppressed?"outside_session":"none");}
#endif
'''
