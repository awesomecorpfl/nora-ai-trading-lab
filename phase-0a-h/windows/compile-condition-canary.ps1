...
$errors=[int]$errorMatches[$errorMatches.Count-1].Groups[1].Value;$warnings=[int]$warningMatches[$warningMatches.Count-1].Groups[1].Value;$size=0;$sha=''
if(Test-Path $ex5){$size=(Get-Item $ex5).Length;if($size -gt 0){$sha=(Get-FileHash $ex5 -Algorithm SHA256).Hash.ToLowerInvariant()}}
$status=if($exitCode -eq 0 -and $errors -eq 0 -and $warnings -eq 0 -and $size -gt 0){'compiled'}else{'failed'}
