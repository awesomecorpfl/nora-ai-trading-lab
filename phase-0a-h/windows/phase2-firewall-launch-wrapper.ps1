...
$tl='C:\NoraEvidence\Phase2\wrapper-trap2.log'
if(Test-Path $tl){Remove-Item $tl -Force}
function TL([string]$m){[IO.File]::AppendAllText($tl,((Get-Date).ToString('HH:mm:ss.fff')+"`t"+$m+"`n"))}
TL('START pid='+$PID+' args='+$PayloadBase64.Substring(0,[Math]::Min(50,$PayloadBase64.Length)))
try{
...
TL('before decode')
try{$p=[Text.Encoding]::UTF8.GetString([Convert]::FromBase64String($PayloadBase64))|ConvertFrom-Json}catch{TL('DECODE FAIL: '+$_.Exception.Message);throw}
TL('after decode evidence_root='+$p.evidence_root+' launch_id='+$p.launch_id)
...
TL('after campaign start pid='+$c.Id)
