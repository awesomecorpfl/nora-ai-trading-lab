...
$submittedSha='unavailable';if($p.PSObject.Properties['submitted_command_sha256'] -and -not [string]::IsNullOrWhiteSpace([string]$p.submitted_command_sha256)){$submittedSha=[string]$p.submitted_command_sha256}
$logicalSha='unavailable';if($p.PSObject.Properties['logical_command_sha256'] -and -not [string]::IsNullOrWhiteSpace([string]$p.logical_command_sha256)){$logicalSha=[string]$p.logical_command_sha256}
