...
function VerifyState($state){
 ...
 for($i=0;$i-lt$bindings.Count;$i++){ $binding=$bindings[$i];[object[]]$rule=@($recordRules|Where-Object{$_.name-eq(RuleName ($i+1))});[object[]]$actual=@($actualViews|Where-Object{$_.name-eq(RuleName ($i+1))});if($rule.Count-ne1 -or $actual.Count-ne1){throw 'missing or duplicate rule GUID binding'};if($rule[0].instance_id-ne$actual[0].instance_id){throw 'rule GUID mismatch'};if($rule[0].program-ne$binding.path -or $recordExecutables[$i].path-ne$binding.path -or $recordExecutables[$i].sha256-ne$binding.sha256){throw 'rule application path or hash mismatch'};if($rule[0].enabled-ne'True' -or $rule[0].direction-ne'Outbound' -or $rule[0].action-ne'Block' -or $rule[0].profile-ne'Any' -or (($rule[0].remote_addresses -join ',') -ne 'Internet')){throw 'containment rule not active'} }
 return $true
}
