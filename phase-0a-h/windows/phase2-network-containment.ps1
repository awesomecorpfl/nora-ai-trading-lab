...
function GetRuleNames {
  param([Parameter(Mandatory=$true)][int]$BindingCount)
  $names=@()
  for($i=1;$i-le$BindingCount;$i++){
    $names += (RuleName $i) + '-Loopback'
    $names += (RuleName $i)
  }
  return $names
}
...
function Get-NoraContainmentRuleViews { param([Parameter(Mandatory=$true)][AllowEmptyCollection()][object[]]$RuleObjects) return @($RuleObjects|ForEach-Object{$rule=$_;[object[]]$apps=@(Get-NetFirewallApplicationFilter -AssociatedNetFirewallRule $rule -ErrorAction Stop);if($apps.Count-ne1){throw 'ambiguous firewall application filter'};[object[]]$addresses=@(Get-NetFirewallAddressFilter -AssociatedNetFirewallRule $rule -ErrorAction Stop);if($addresses.Count-ne1){throw 'ambiguous firewall address filter'};[ordered]@{name=$rule.Name;instance_id=[string]$rule.InstanceID;display_name=$rule.DisplayName;group=$rule.Group;enabled=[string]$rule.Enabled;direction=[string]$rule.Direction;action=[string]$rule.Action;profile=[string]$rule.Profile;program=$apps[0].Program;remote_addresses=@([string]$addresses[0].RemoteAddress)}}|Sort-Object program,name,instance_id) }
...
function AssertNoContradictoryAllow($bindings){$allows=@(Get-NetFirewallRule -Direction Outbound -Enabled True -Action Allow -ErrorAction Stop|Where-Object{[string]$_.Group -ne $firewallGroup}|ForEach-Object{$r=$_;Get-NetFirewallApplicationFilter -AssociatedNetFirewallRule $r -ErrorAction SilentlyContinue|ForEach-Object{[string]$_.Program}});foreach($binding in $bindings){if($allows -contains $binding.path){throw 'contradictory MT5 outbound allow rule'}}}
...
function VerifyState($state){
 ...
 [object[]]$bindings=@(Bindings);[object[]]$recordExecutables=@($state.executables);[object[]]$recordRules=@($state.rules);[object[]]$actualRules=@(Get-NoraContainmentRules -CampaignId $CampaignId);[object[]]$actualViews=@(Get-NoraContainmentRuleViews -RuleObjects $actualRules)
 # Expect 2 rules per executable (Allow Loopback + Block Internet)
 $expectedRuleCount = $bindings.Count * 2
 if($recordExecutables.Count-ne$bindings.Count -or $recordRules.Count-ne$expectedRuleCount -or $actualViews.Count-ne$expectedRuleCount){throw ('containment cardinality mismatch: expected '+$expectedRuleCount+' rules, got '+$recordRules.Count+' recorded, '+$actualViews.Count+' actual')}
 AssertNoContradictoryAllow $bindings
 for($i=0;$i-lt$bindings.Count;$i++){ $binding=$bindings[$i];$idx=$i+1
   # Verify Allow Loopback rule
   $allowName = (RuleName $idx) + '-Loopback'
   [object[]]$allowRecorded=@($recordRules|Where-Object{$_.name-eq$allowName})
   [object[]]$allowActual=@($actualViews|Where-Object{$_.name-eq$allowName})
   if($allowRecorded.Count-ne1 -or $allowActual.Count-ne1){throw ('missing or duplicate Allow Loopback rule for '+$allowName)}
   if($allowRecorded[0].instance_id-ne$allowActual[0].instance_id){throw 'Allow Loopback rule GUID mismatch'}
   if($allowRecorded[0].program-ne$binding.path){throw 'Allow Loopback rule application path mismatch'}
   if($allowRecorded[0].enabled-ne'True' -or $allowRecorded[0].direction-ne'Outbound' -or $allowRecorded[0].action-ne'Allow' -or $allowRecorded[0].profile-ne'Any'){throw 'Allow Loopback rule not active'}
   $allowRemotes=@($allowRecorded[0].remote_addresses)
   if($allowRemotes.Count-ne1 -or $allowRemotes[0]-ne'127.0.0.1'){throw ('Allow Loopback rule remote addresses mismatch: '+($allowRemotes -join ','))}
   
   # Verify Block Internet rule
   $blockName = (RuleName $idx)
   [object[]]$blockRecorded=@($recordRules|Where-Object{$_.name-eq$blockName})
   [object[]]$blockActual=@($actualViews|Where-Object{$_.name-eq$blockName})
   if($blockRecorded.Count-ne1 -or $blockActual.Count-ne1){throw ('missing or duplicate Block Internet rule for '+$blockName)}
   if($blockRecorded[0].instance_id-ne$blockActual[0].instance_id){throw 'Block Internet rule GUID mismatch'}
   if($blockRecorded[0].program-ne$binding.path -or $recordExecutables[$i].path-ne$binding.path -or $recordExecutables[$i].sha256-ne$binding.sha256){throw 'Block Internet rule application path or hash mismatch'}
   if($blockRecorded[0].enabled-ne'True' -or $blockRecorded[0].direction-ne'Outbound' -or $blockRecorded[0].action-ne'Block' -or $blockRecorded[0].profile-ne'Any'){throw 'Block Internet rule not active'}
   $blockRemotes=@($blockRecorded[0].remote_addresses)
   if($blockRemotes.Count-ne1 -or $blockRemotes[0]-ne'Internet'){throw ('Block Internet rule remote addresses mismatch: '+($blockRemotes -join ','))}
 }
 return $true
}
...
try {
 switch($Action){
  'stage' {
   [object[]]$bindings=@(Bindings);[object[]]$existingRules=@(Get-NoraContainmentRules -CampaignId $CampaignId);$intent=[ordered]@{schema_version=$schema;phase='intent_prepared';campaign_identity=$CampaignId;repository_commit=$env:NORA_REPOSITORY_COMMIT;creator=Identity;executables=@($bindings);rules=@($existingRules);group=$firewallGroup;intended_rule_names=@(for($i=1;$i-le($bindings.Count*2);$i++){RuleName $i});intended_final_record_path=FinalPath;evidence_root=$EvidenceRoot;captured_utc=(Get-Date).ToUniversalTime().ToString('o')}
   ...
   AssertNoContradictoryAllow $bindings
   # Phase 1: Allow loopback for each executable (high priority)
     $i=0;foreach($binding in $bindings){
       $i++;
       $allowName = (RuleName $i) + '-Loopback';
       # Use IPv4 loopback only; ::1 causes "unspecified/multicast/broadcast/loopback IPv6 address" error
       New-NetFirewallRule -Name $allowName -DisplayName $allowName -Group $firewallGroup -Direction Outbound -Action Allow -Program $binding.path -RemoteAddress 127.0.0.1 -Profile Any -Enabled True -ErrorAction Stop | Out-Null;
       if($Fault-eq'after_first_rule' -and $i-eq1){throw 'forced failure after first rule'}
     }
   # Phase 2: Block Internet for each executable (lower priority)
   $i=0;foreach($binding in $bindings){
     $i++;
     $blockName = (RuleName $i);
     New-NetFirewallRule -Name $blockName -DisplayName $blockName -Group $firewallGroup -Direction Outbound -Action Block -Program $binding.path -RemoteAddress Internet -Profile Any -Enabled True -ErrorAction Stop | Out-Null;
     if($Fault-eq'after_all_rules_before_final' -and $i-eq$bindings.Count){throw 'forced failure after all rules before final publication'}
   }
