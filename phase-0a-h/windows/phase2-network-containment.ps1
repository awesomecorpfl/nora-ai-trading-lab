...
function FreshVerify(){ $args=@('-NoProfile','-NonInteractive','-ExecutionPolicy','Bypass','-File',$PSCommandPath,'-Action','verify-final','-CampaignId',$CampaignId,'-EvidenceRoot',$EvidenceRoot,'-InstallRoot',$InstallRoot);foreach($p in $normalizedExecutablePaths){$args+=@('-ExecutablePath',$p)};$output=@(& powershell.exe @args 2>&1);if($LASTEXITCODE-ne0){throw ('fresh record validation failed: '+($output -join "`n"))};return ($output -join "`n")}
try {
 switch($Action){
  'stage' {
  ...
  'recover' {[object[]]$rules=@(Get-NoraContainmentRules -CampaignId $CampaignId);$classification=if($rules.Count-eq0 -and !(Test-Path -LiteralPath (IntentPath))){'NO_RULES_NO_RECORD'}elseif($rules.Count-eq0){'NO_RULES_TRANSACTION_FAILED'}elseif(!(Test-Path -LiteralPath (FinalPath))){'RULES_PRESENT_RECORD_INCOMPLETE'}else{'RECORD_PRESENT_REQUIRES_VERIFY'};$v=[ordered]@{schema_version=$schema;campaign_identity=$CampaignId;classification=$classification;active_rule_count=$rules.Count;rules=@($rules);intent_present=(Test-Path -LiteralPath (IntentPath));final_record_present=(Test-Path -LiteralPath (FinalPath));accepted_record_present=(Test-Path -LiteralPath (AcceptedPath));recovered_utc=(Get-Date).ToUniversalTime().ToString('o')};AtomicJson (RecoveryPath) $v;$v|ConvertTo-Json -Depth 20 -Compress}
