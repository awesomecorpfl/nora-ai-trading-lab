...
# Phase-2R durable condition tester compile helper.
# Mirrors phase-0a-h/windows/compile-slope-tester-canary.ps1 capture fidelity so
# the durable compile manifest carries rendered_command, freshness flags, source
# hashes, and the nested native_evidence object the Phase-2N standard requires.
# Does NOT modify the frozen compile-condition-tester-canary.ps1.
...
foreach($name in @('NoraPhase2RuntimeV1.mqh','NoraPhase2ConditionV1.mqh','NoraPhase2ConditionTesterCanaryV1.mq5')){
    Copy-Item (Join-Path $IncomingRoot $name) (Join-Path $source $name) -Force
}

# Compute source hashes on Windows so the compile manifest binds them at the
# point of compilation, matching the slope helper's contract.
$runtimeSrc = Join-Path $source 'NoraPhase2RuntimeV1.mqh'
$conditionSrc = Join-Path $source 'NoraPhase2ConditionV1.mqh'
$testerSrc = Join-Path $source 'NoraPhase2ConditionTesterCanaryV1.mq5'
...
$sourceEx5=Join-Path $source 'NoraPhase2ConditionTesterCanaryV1.ex5'
$out=Join-Path $run 'NoraPhase2ConditionTesterCanaryV1.ex5'
...
# Frozen source hashes (must match the accepted fixture constants).
...
if($testerSha -ne $TESTER_SOURCE_SHA256){ throw "tester source hash mismatch: $testerSha" }
...
$result=[ordered]@{
    ...
    ex5_filename='NoraPhase2ConditionTesterCanaryV1.ex5'
    runtime_source_sha256=$runtimeSha
    condition_source_sha256=$conditionSha
    tester_source_sha256=$testerSha
}

# Embed a nested native_evidence object that mirrors the slope compile manifest
# shape, so the Phase-2R RED test can use the same access patterns.
