...
$root="$env:USERPROFILE\NoraPhase2L"
$run=Join-Path $root $RunId
$source=Join-Path $run 'source'
New-Item -ItemType Directory -Force $source|Out-Null
foreach($name in @('NoraPhase2RuntimeV1.mqh','NoraPhase2ConditionV1.mqh','NoraPhase2SeriesRuntimeV1.mqh','NoraPhase2SeriesTesterCanaryV1.mq5')){Copy-Item (Join-Path $IncomingRoot $name) (Join-Path $source $name) -Force}
...
$program=Join-Path $source 'NoraPhase2SeriesTesterCanaryV1.mq5'
$log=Join-Path $run 'compile.log'
$ex5=Join-Path $source 'NoraPhase2SeriesTesterCanaryV1.ex5'
...
$result=[ordered]@{status=$status;compiler_path=$editor;compiler_version=(Get-Item $editor).VersionInfo.FileVersion;compiler_exit_code=$p.ExitCode;error_count=$errors;warning_count=$warnings;ex5_filename='NoraPhase2SeriesTesterCanaryV1.ex5';ex5_size_bytes=$size;ex5_sha256=$sha}
